/*  ECE-4730     : Introduction to Parallel Systems
	...
	Description: This program writes to output_file a binary file with an n by n (square) matrix representing one of
				 ...
				 For the matrices, n = m, but the read/write routine should use the standard of two int anyway. You also need to
				modify your matrix creating algorithm to create a matrix that has large values at and/or near the diagonal, to ensure
				the matrix is solvable (if for a[i,j] where i = j then a[i,j] = a[i,j] * 100).
